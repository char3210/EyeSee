package xyz.duncanruns.eyesee.win32;
...
public final class KeyboardUtil {
    ...
    private static List<Object> virtualKeyToScanCode(int virtualKey) {
        ...
        return Arrays.asList(scanCode, isExtended);
    }
    ...
    private static WinDef.LPARAM createLParam(int virtualKey, int repeatCount, boolean transitionState, boolean previousKeyState, boolean contextCode) {
        List<Object> scanCode = virtualKeyToScanCode(virtualKey);
        return new WinDef.LPARAM(((transitionState ? 1 : 0) << 31) | ((previousKeyState ? 1 : 0) << 30) | ((contextCode ? 1 : 0) << 29) | ((((boolean) scanCode.get(1)) ? 1 : 0) << 24) | ((int) scanCode.get(0) << 16) | (repeatCount));
    }
